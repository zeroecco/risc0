...
CircuitInfo* getCircuitInfo(IHalPtr hal, size_t po2) {
  ...
  // Set up taps
  // NOTE: Calling addTap for a given group and column index will result in that column being
  // defined in the circuit info. Below, in the witgen function, the access to that column will work
  // if that column is defined here.
  // NOTE: This must match the taps defined in the verify info below.
  // NOTE: The order of calls to `addTap` does not matter, as they will be sorted, by (group, col,
  // back), and deduplicated in `done`.
  ...
  ci.evalCheck = [hal, po2](HalMatrix<Fp> out, std::vector<GroupState>& state, FpExt ecMix) {
    ...
    for (size_t row = 0; row < NUM_ROWS; row++) {
      ...
      // NOTE: This must match witgen and the verify eval check.
...
VerifyCircuitInfo* getVerifyCircuitInfo() {
  ...
  ci.evalCheck = [](FpExt* evals, Fp* _globals, FpExt* _mix, FpExt ecMix, FpExt _z) -> FpExt {
    // NOTE: evals is a vector with all tap evaluations in it. Taps are ordered by the tuple (group,
    // col, back). The +1 and +2 slicing of the vector is based on the fact that the accum and data
    // groups are each of size 1 (s.t. size of code + size of accum is 2).
    ...
    // NOTE: Add constraints. This must match what is generated by witgen callbacks above.
