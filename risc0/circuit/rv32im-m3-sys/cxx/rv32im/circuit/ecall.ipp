...
template<typename C>
FDEV void EcallTerminateBlock<C>::set(CTX, EcallTerminateWitness wit) DEV {
...
template<typename C>
FDEV void EcallTerminateBlock<C>::verify(CTX) DEV {
...
template<typename C>
FDEV void EcallTerminateBlock<C>::addArguments(CTX) DEV {
...
template<typename C>
FDEV void EcallReadBlock<C>::set(CTX, EcallReadWitness wit) DEV {
  cycle.set(ctx, wit.cycle);
...
template<typename C>
FDEV void EcallReadBlock<C>::verify(CTX) DEV {
...
template<typename C>
FDEV void EcallReadBlock<C>::addArguments(CTX) DEV {
  Val<C> cycleVal = cycle.get();
...
template<typename C>
FDEV void EcallWriteBlock<C>::set(CTX, EcallWriteWitness wit) DEV {
  cycle.set(ctx, wit.cycle);
  fetch.set(ctx, wit.fetch);
  readA7.set(ctx, wit.a7, wit.cycle);
  readA2.set(ctx, wit.a2, wit.cycle);
...
template<typename C>
FDEV void EcallWriteBlock<C>::verify(CTX) DEV {
  // Make sure A7 = HOST_ECALL_READ
  EQ(readA7.wordAddr.get(), MACHINE_REGS_WORD + REG_A7);
...
template<typename C>
FDEV void EcallWriteBlock<C>::addArguments(CTX) DEV {
  Val<C> cycleVal = cycle.get();
  ctx.pull(CpuStateArgument<C>(cycleVal, fetch.pc.get(), 1, fetch.iCacheCycle.get()));
...
template<typename C>
FDEV void EcallBigIntBlock<C>::set(CTX, EcallBigIntWitness wit) DEV {
  cycle.set(ctx, wit.cycle);
  fetch.set(ctx, wit.fetch);
  readA7.set(ctx, wit.a7, wit.cycle);
...
template<typename C>
FDEV void EcallBigIntBlock<C>::verify(CTX) DEV {
...
template<typename C>
FDEV void EcallBigIntBlock<C>::addArguments(CTX) DEV {
  Val<C> cycleVal = cycle.get();
  ...
  ctx.push(CpuStateArgument<C>(cycleVal + countVal + 2, fetch.nextPc.get(), 1, fetch.iCacheCycle.get()));
  VERIFY_DECODE
}
