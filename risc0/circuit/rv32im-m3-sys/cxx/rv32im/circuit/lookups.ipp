...
template<typename C>
FDEV Val<C> RegU8<C>::get() DEV {
  return inner.get();
}

template<typename C>
FDEV void RegU8<C>::addArguments(CTX) DEV {
  ctx.pull(LookupArgument<C>(0, inner.get()));
}

template<typename C>
FDEV void RegU16<C>::set(CTX, Val<C> in) DEV {
  inner.set(ctx, in);
  uint32_t val = in.asUInt32();
  // This shouldn't happen, but make sure we don't corrupt memory
  if (val >= 65536) {
...
template<typename C>
FDEV Val<C> RegU16<C>::get() DEV {
  return inner.get();
}

template<typename C>
FDEV void RegU16<C>::addArguments(CTX) DEV {
  ctx.pull(LookupArgument<C>(1, inner.get()));
}

template<typename C>
FDEV void MakeTableBlock<C>::set(CTX, MakeTableWitness wit) DEV {
  table.set(ctx, wit.table);
  start.set(ctx, wit.start);
}

template<typename C>
FDEV void MakeTableBlock<C>::finalize(CTX) DEV {
  uint32_t theTable = table.get().asUInt32();
  uint32_t pos = start.get().asUInt32();
  if (theTable > 0) { pos += 256; }
  if (theTable > 1) { pos += 65536; }
...
template<typename C>
FDEV void MakeTableBlock<C>::verify(CTX) DEV {}

template<typename C>
FDEV void MakeTableBlock<C>::addArguments(CTX) DEV {
