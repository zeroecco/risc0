...
pub trait SegmentUpdateCallback {
    fn segment_update_callback(&mut self, update: SegmentUpdate) -> Result<()>;
}
...
impl<'a, 'b, S: Syscall> Executor<'a, 'b, S> {
    ...
    pub fn run(
        ...
        // TODO(victor/perf): Keep the Send requirement? Its technically not required if we are
        // going to use only one thread here.
        mut callback: impl SegmentUpdateCallback + Send,
    ...
    fn split_segment(
        ...
        callback
            .segment_update_callback(update)
